/*
  HugeNumbers_I2C.cpp - Library for displaying huge (4*3) numbers on LCD displays with I2C using the HD44780 driver.
 
  Copyright (C) 2019 Anush DP
  Copyright (C) 2020 Feng Zhou
...
// Creates HugeNumbers_I2C object
// LiquidCrystal_I2C* lcd: LiquidCrystal object to use
HugeNumbers_I2C::HugeNumbers_I2C(LiquidCrystal_I2C* lcd)
{
  _lcd = lcd;
}

void HugeNumbers_I2C::begin()
{
...
void HugeNumbers_I2C::displayHugeInt(int n, byte x, byte y, byte digits, bool leading)
{
  ...
  for (int i = 0; i < digits; i++)
  {
    ...
    {
      clearHugeNumber((i * 4) + x, y);
    }
    else
    {
    displayHugeNumber(numString[i], (i * 4) + x, y);
      leading = true;
    }
  }
}

void HugeNumbers_I2C::clearHugeNumber(byte x, byte y) /* x is column of upper left corner for large character y is row of upper left corner for large character */
{
  _lcd->setCursor(x,y);
  _lcd->print(F("    "));
  _lcd->setCursor(x,y+1); 
  _lcd->print(F("    "));
  _lcd->setCursor(x,y+2); 
  _lcd->print(F("    "));
}

void HugeNumbers_I2C::displayHugeNumber(byte n, byte x, byte y) /* n is number to display, x is column of upper left corner for large character
